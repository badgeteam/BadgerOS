...
fn alloc_pgtable_page(alloc: &dyn PhysAlloc) -> EResult<PPN> {
    let ppn = alloc.page_alloc(0).ok_or(Errno::ENOMEM)?;
...
fn split_pgtable_leaf(orig: PTE, new_level: u8, alloc: &dyn PhysAlloc) -> EResult<PPN> {
...
pub unsafe fn map(
    ...
    for level in (target_level + 1..unsafe { PAGING_LEVELS }).rev() {
        ...
        pgtable_ppn = if !pte.valid {
            ...
            if new_pte.flags == 0 && new_pte.ppn == 0 {
                // Unless the invalid PTE is null.
                return Ok(());
            }
            let ppn = alloc_pgtable_page(alloc)?;
            unsafe {
                write_pte(
                    pgtable_ppn,
                    index,
                    PTE {
                        ppn,
                        flags: new_pte.flags & super::flags::G,
                        valid: true,
                        leaf: false,
                        level,
                    }
            ...
            let ppn = split_pgtable_leaf(pte, level - 1, alloc)?;
            unsafe {
                write_pte(
                    pgtable_ppn,
                    index,
