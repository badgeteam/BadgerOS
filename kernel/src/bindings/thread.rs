...
use super::raw::{self, SCHED_PRIO_NORMAL, badge_err_t, tid_t, timestamp_us_t};
...
impl Thread {
    ...
    pub fn try_new<T: FnOnce() -> i32 + Send + 'static>(
        ...
        unsafe {
            ...
            if tid == 0 {
                ...
                raw::thread_resume(0 as *mut badge_err_t, tid);
                Some(Self { handle: tid })
            }
        }
    }
    ...
    pub fn detach(self) {
        unsafe { raw::thread_detach(0 as *mut badge_err_t, self.handle) }
    }
}
